...
export const useApiSimple = () => {
  ...
  const art = {
    ...
    like: (id) => callApi(`/art/${id}/like`, { method: 'POST' }),
    unlike: (id) => callApi(`/art/${id}/unlike`, { method: 'POST' }),
    favorite: (id) => callApi(`/art/${id}/favorite`, { method: 'POST' }),
    unfavorite: (id) => callApi(`/art/${id}/unfavorite`, { method: 'POST' }),
    getFavorites: () => callApi('/art/favorites'),
    getLikes: () => callApi('/art/likes'),
  };

  // 活动相关API
  const activity = {
    getAll: () => callApi('/activities'),
    ...
    join: (id) => callApi(`/activities/${id}/join`, { method: 'POST' }),
    leave: (id) => callApi(`/activities/${id}/leave`, { method: 'POST' }),
  };

  // 反馈相关API
  const feedback = {
    getAll: () => callApi('/feedback'),
